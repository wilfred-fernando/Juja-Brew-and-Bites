...
function orderStatusMessage(order, status) {
  ...
  if (status === "ready") {
    return {
      title: "Order Ready",
      body: `Order #${shortId} is ready for pickup or delivery.`,
    };
  }
  return {
...
export async function sendCustomerOrderStatusPush({ webOrderId, status }) {
  ...
  if (!["ready", "completed", "delivered"].includes(normalizedStatus)) {
  ...
  const { data: order, error: orderError } = await admin
    .from("web_orders")
    .select("id,user_id,receipt_number,order_number,status,total,subtotal")
    .eq("id", webOrderId)
    .maybeSingle();
