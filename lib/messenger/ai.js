...
import { stripFoodBookingMenuLinks } from "@/lib/messenger/replySanitizer";
...
- Pasong Tamo Branch: 36D Visayas Ave., Pasong Tamo, Quezon City. Open daily, 10 AM to 12 midnight.
- Diliman Branch: 8 Visayas Ave., Diliman, Quezon City. Monday to Saturday, 9 AM to 10 PM; closed Sunday.
...
- When the customer asks about the menu, food, drinks, flavors, or prices, answer from the live menu reference without sharing a menu link.
...
export async function generateMessengerAiReply({ psid, firstName = "", history = [], instructions = "" }) {
  ...
  const text = stripFoodBookingMenuLinks(ensureFirstName(parsed.text, customerFirstName)).slice(0, 1800);
