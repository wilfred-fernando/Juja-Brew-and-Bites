...
import { getMessengerProfile, MESSENGER_BOT_METADATA, replyForMessengerEvent, sendMessengerMessage } from "@/lib/messenger";
import { generateMessengerAiReply } from "@/lib/messenger/ai";
import { loadMessengerAiContext } from "@/lib/messenger/context";
import { sanitizeMessengerReply } from "@/lib/messenger/replySanitizer";

const MAX_NODE_STEPS = 20;
const HUMAN_REPLY_PAUSE_MS = 60 * 60 * 1000;
const AI_AGENT_NODE = { id: null, node_type: "ai", config: {} };
const HANDOFF_NODE = {
  id: null,
  node_type: "handoff",
  config: {
    text: "I’ve paused automatic replies so a JUJA Brew & Bites Live Chat agent can assist you. Please leave the details here and our team will review them.",
  },
};
...
function routingMode() {
  return clean(process.env.MESSENGER_ROUTING_MODE).toLowerCase() === "flows" ? "flows" : "agent";
}

function isHandoffRequest(input) {
  const value = clean(input.payload || input.text).toLowerCase().replace(/[.!?]+$/g, "");
  return new Set(["live chat", "livechat", "staff", "human", "agent", "representative", "person", "tao", "handoff"]).has(value);
}
...
function inboundEvent(event) {
  ...
  return {
    ...
    eventType,
    payload,
    pageId: clean(event?.recipient?.id),
    psid,
    text,
  };
}
...
function nodeMessage(node, context) {
  ...
  if (buttons.length) {
    return sanitizeMessengerReply({ attachment: { type: "template", payload: { template_type: "button", text, buttons } } });
  }
  return sanitizeMessengerReply({ text, ...(quickReplies.length ? { quick_replies: quickReplies } : {}) });
}

async function recordOutbound(admin, psid, pageId, node, message, result) {
...
async function sendNodeMessage(admin, contact, node, context) {
  const message = nodeMessage(node, context);
  if (!message.text && !message.attachment) return null;
  const result = await sendMessengerMessage(contact.psid, message, { pageId: contact.page_id });
  await recordOutbound(admin, contact.psid, contact.page_id, node, message, result);
  return result;
}

function eventHistoryItem(row) {
  if (row.direction === "inbound") {
    const content = clean(row.payload?.message?.text || row.payload?.postback?.title || row.payload?.postback?.payload);
    return content ? { role: "user", content } : null;
  }
  const content = clean(row.payload?.message?.text || row.payload?.message?.attachment?.payload?.text);
  return content ? { role: "assistant", content } : null;
}

async function aiHistory(admin, psid) {
  ...
  return (data || []).reverse().map(eventHistoryItem).filter(Boolean);
}

async function sendAiNodeMessage(admin, contact, node) {
  const config = node.config || {};
  let aiResult;
  let usedFallback = false;
  try {
    const history = await aiHistory(admin, contact.psid);
    const latestQuestion = [...history].reverse().find((item) => item.role === "user")?.content || "";
    const businessContext = await loadMessengerAiContext(admin, { query: latestQuestion });
    aiResult = await generateMessengerAiReply({
      psid: contact.psid,
      firstName: contact.first_name,
      history,
      instructions: [clean(config.instructions), businessContext.instructions].filter(Boolean).join("\n\n"),
    });
  } catch (error) {
    console.error("Messenger AI reply failed:", error);
    usedFallback = true;
    aiResult = { text: clean(config.fallback_text) || "I’m unable to answer that automatically right now. Type “Live Chat” and our team will help you." };
  }
  ...
  const message = sanitizeMessengerReply({ text: aiResult.text, ...(quickReplies.length ? { quick_replies: quickReplies } : {}) });
  const result = await sendMessengerMessage(contact.psid, message, { pageId: contact.page_id });
  await recordOutbound(admin, contact.psid, contact.page_id, node, message, {
    ...result,
    ai: { model: aiResult.model || null, response_id: aiResult.responseId || null, usage: aiResult.usage || null, fallback: usedFallback },
  });
  return { ...result, orderHandoffRequested: Boolean(aiResult.orderHandoffRequested) };
}

async function pauseForOrderVerification(admin, contact) {
  const pausedAt = new Date().toISOString();
  const tags = [...new Set([...(contact.tags || []), "messenger-order-draft"])];
  const { error } = await admin.from("messenger_contacts").update({
    bot_paused: true,
    pause_reason: "Draft order awaiting Live Chat verification",
    paused_at: pausedAt,
    auto_resume_at: null,
    tags,
  }).eq("psid", contact.psid);
  if (error) throw error;
  return { ...contact, bot_paused: true, paused_at: pausedAt, auto_resume_at: null, tags };
}

async function enrichContactProfile(admin, contact) {
  if (clean(contact?.first_name)) return contact;
  try {
    const profile = await getMessengerProfile(contact.psid, { pageId: contact.page_id });
    const firstName = clean(profile?.first_name).slice(0, 120);
    const lastName = clean(profile?.last_name).slice(0, 120);
    const displayName = clean(profile?.name || [firstName, lastName].filter(Boolean).join(" ")).slice(0, 240);
    if (!firstName && !displayName) return contact;
    const updates = {
      first_name: firstName || null,
      last_name: lastName || null,
      display_name: displayName || firstName,
      locale: clean(profile?.locale).slice(0, 32) || null,
      timezone: Number.isFinite(Number(profile?.timezone)) ? Number(profile.timezone) : null,
    };
    const { data, error } = await admin.from("messenger_contacts").update(updates).eq("psid", contact.psid).select("*").single();
    if (error) throw error;
    return data || { ...contact, ...updates };
  } catch {
    console.warn("Unable to load Messenger customer profile; continuing without profile enrichment.");
    return contact;
  }
}

async function routeToAiAgent(admin, contact, input) {
  if (isHandoffRequest(input)) {
    const pausedAt = new Date().toISOString();
    const { error } = await admin.from("messenger_contacts").update({
      bot_paused: true,
      pause_reason: "Customer requested Live Chat",
      paused_at: pausedAt,
      auto_resume_at: null,
    }).eq("psid", contact.psid);
    if (error) throw error;
    await sendNodeMessage(admin, { ...contact, bot_paused: true, paused_at: pausedAt }, HANDOFF_NODE, {});
  ...
  if (!input.text && !input.payload) {
    const node = {
      id: null,
      node_type: "message",
      config: { text: "Please type your question and I’ll be happy to help." },
    };
    await sendNodeMessage(admin, contact, node, {});
    return { sent: true, requestedText: true };
  }

  const aiOutcome = await sendAiNodeMessage(admin, contact, AI_AGENT_NODE);
  if (aiOutcome.orderHandoffRequested) {
    await pauseForOrderVerification(admin, contact);
    return { sent: true, ai: true, handedOff: true, orderDraft: true };
  }
  return { sent: true, ai: true };
}
...
async function executeFlow(admin, contact, session, startNodeKey) {
  ...
  for (let step = 0; step < MAX_NODE_STEPS; step += 1) {
    ...
    if (node.node_type === "message") {
      await sendNodeMessage(admin, currentContact, node, context);
    } else if (node.node_type === "ai") {
      const aiOutcome = await sendAiNodeMessage(admin, currentContact, node);
      if (aiOutcome.orderHandoffRequested) {
        currentContact = await pauseForOrderVerification(admin, currentContact);
        await finishSession(admin, currentSession.id, "paused");
        return { handedOff: true, orderDraft: true };
      }
      ...
      const { error: contactError } = await admin.from("messenger_contacts").update({
        bot_paused: true,
        pause_reason: clean(node.config?.reason) || "Customer requested Live Chat",
        paused_at: pausedAt,
        auto_resume_at: null,
...
async function handlePageMessageEcho(event) {
  ...
  const pageId = clean(event?.sender?.id);
  const psid = clean(event?.recipient?.id);
  const messageId = clean(event?.message?.mid);
  if (!pageId || !psid) return { ignored: true, echo: true };

  const admin = adminClient();
  if (clean(event?.message?.metadata) === MESSENGER_BOT_METADATA) {
    return { ignored: true, botEcho: true };
  }
  ...
  if (messageId) {
    const { data, error } = await admin
      ...
      .maybeSingle();
    if (error) throw error;
    if (data) return { ignored: true, botEcho: true };
  }

  const now = new Date();
  const autoResumeAt = new Date(now.getTime() + HUMAN_REPLY_PAUSE_MS).toISOString();
  const { error: contactError } = await admin.from("messenger_contacts").upsert({
    psid,
    page_id: pageId,
    bot_paused: true,
    pause_reason: "Human replied from Page Inbox",
  ...
  const eventId = messageId || `human-echo:${createHash("sha256").update(JSON.stringify(event)).digest("hex")}`;
  ...
  return { paused: true, humanReply: true, autoResumeAt };
}

async function resumeExpiredHumanPause(admin, contact, now = new Date()) {
  if (!contact?.bot_paused || !contact?.auto_resume_at) return contact;
  const resumeAt = new Date(contact.auto_resume_at);
  if (!Number.isFinite(resumeAt.getTime()) || resumeAt > now) return contact;
  const updates = {
    bot_paused: false,
    pause_reason: null,
    paused_at: null,
...
async function routeEventWithDatabase(event) {
  const echoOutcome = await handlePageMessageEcho(event);
  if (echoOutcome) return echoOutcome;
  const input = inboundEvent(event);
  if (!input.psid || event?.delivery || event?.read || event?.reaction) return { ignored: true };
  const admin = adminClient();
  const now = new Date().toISOString();

  const { data: storedContact, error: contactError } = await admin.from("messenger_contacts").upsert({
    psid: input.psid,
    page_id: input.pageId || null,
    last_message_at: now,
  }, { onConflict: "psid" }).select("*").single();
  if (contactError) throw contactError;
  const enrichedContact = await enrichContactProfile(admin, storedContact);
  const contact = await resumeExpiredHumanPause(admin, enrichedContact, new Date(now));
  ...
  try {
    if (routingMode() === "agent") {
      const result = await routeToAiAgent(admin, contact, input);
...
async function fallbackRoute(event) {
  ...
  const pageId = clean(event?.recipient?.id);
  const message = sanitizeMessengerReply(replyForMessengerEvent(event));
  if (!recipientId || !message) return { ignored: true };
  await sendMessengerMessage(recipientId, message, { pageId });
...
export async function processMessengerWebhook(payload) {
  ...
  return {
    ...
    humanReplies: outcomes.filter((outcome) => outcome?.humanReply).length,
  };
}
