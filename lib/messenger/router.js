...
const HANDOFF_NODE = {
  ...
  node_type: "handoff",
  config: {
    text: "I’ve paused automatic replies so a JUJA Brew & Bites team member can assist you. Please leave the details here and our staff will review them.",
  },
};
...
function isHandoffRequest(input) {
  ...
  return new Set(["staff", "human", "agent", "representative", "person", "tao", "handoff"]).has(value);
}
...
async function sendAiNodeMessage(admin, contact, node) {
  ...
  try {
    ...
    usedFallback = true;
    aiResult = { text: clean(config.fallback_text) || "I’m unable to answer that automatically right now. Type “staff” and our team will help you." };
  }
...
async function routeToAiAgent(admin, contact, input) {
  if (isHandoffRequest(input)) {
    ...
    const { error } = await admin.from("messenger_contacts").update({
      bot_paused: true,
      pause_reason: "Customer requested staff",
      paused_at: pausedAt,
      auto_resume_at: null,
