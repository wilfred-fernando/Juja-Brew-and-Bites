export function requiredBeneficiaryTypeForRule(rule) {
  ...
  const configuredType = String(rule?.promotion_rules?.beneficiary_type || rule?.promotionRules?.beneficiary_type || "").toLowerCase();
  if (configuredType === "teacher" || /\bteacher/.test(name)) return "teacher";
...
export function beneficiaryMatchesRule(type, requiredType) {
  ...
  if (requiredType === "teacher") return type === "teacher";
