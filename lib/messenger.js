...
export async function sendMessengerMessage(recipientId, message) {
  const { graphApiVersion, pageAccessToken } = messengerConfig();
  if (!graphApiVersion || !pageAccessToken) {
    throw new Error("Messenger Graph API version and Page access token are required.");
  }
