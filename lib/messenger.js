...
export const MESSENGER_BOT_METADATA = "jujabot:auto-reply";
...
function quickReplies() {
  return [
    ...
    { content_type: "text", title: "Live Chat", payload: "HUMAN_HELP" },
  ];
}
...
function orderMessage(orderUrl) {
  if (!orderUrl) {
    return textMessage(
      "Online ordering is not linked yet. Please choose Live Chat and our team will help you place an order.",
      true
    );
  }
...
export function replyForMessengerEvent(event) {
  ...
  if (payload === "HUMAN_HELP" || /\b(human|staff|agent|person|complaint|problem|help|live chat|livechat)\b/.test(text)) {
    return textMessage(
      "A Live Chat agent can continue this conversation in Messenger. Please send your name, preferred branch, and what you need help with.",
      true
    );
  }

  if (event?.message?.attachments?.length) {
    return textMessage("Thanks for the attachment. Please add a short message explaining how our Live Chat team can help.", true);
  }

  return textMessage("I can help with our menu, online ordering, store details, or connect you with Live Chat.", true);
}
...
export async function sendMessengerMessage(recipientId, message, { pageId = "" } = {}) {
  ...
  const response = await fetch(`${GRAPH_API_ORIGIN}/${graphApiVersion}/me/messages`, {
    ...
    body: JSON.stringify({
      ...
      message: { ...message, metadata: MESSENGER_BOT_METADATA },
    }),
  });
