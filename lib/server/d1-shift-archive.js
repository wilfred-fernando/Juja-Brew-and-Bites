...
function orderAmounts(row) {
  ...
  const storedNet = row.net_amount == null ? gross - discounts : num(row.net_amount);
  const net = Math.max(0, Math.min(storedNet, gross - discounts - refunds));
...
function archiveAudit(row, entity, namespace, businessDate) {
  return {
    source_id: `${namespace}:${row.id}`,
    business_date: businessDate,
    store_id: row.store_id || null,
    actor_user_id: row.actor_id || row.actor_user_id || row.user_id || null,
    entity,
    action: row.action || null,
    created_at: manilaIso(row.created_at || new Date()),
...
export async function archiveClosedShift(shiftId) {
  ...
  try {
  const [ordersRaw, webRaw, savedTicketAuditRaw, posCartAuditRaw] = await Promise.all([
    ...
    fetchRows(client, "saved_ticket_audit_logs", (query) => query.eq("store_id", shift.store_id).gte("created_at", openedAt).lte("created_at", closedAt)),
    fetchRows(client, "pos_cart_audit_logs", (query) => query.eq("store_id", shift.store_id).gte("created_at", openedAt).lte("created_at", closedAt)),
  ]);
  ...
  const archivedSavedTicketAudit = savedTicketAuditRaw.map((row) => archiveAudit(row, "saved_ticket", "saved_ticket", manilaDate(row.created_at)));
  const archivedPosCartAudit = posCartAuditRaw.map((row) => archiveAudit(row, "pos_cart", "pos_cart", manilaDate(row.created_at)));
  const datasets = [
    ...
    { table: "archive_audit_logs", manifestTable: "archive_saved_ticket_audit_logs", rows: archivedSavedTicketAudit },
    { table: "archive_audit_logs", manifestTable: "archive_pos_cart_audit_logs", rows: archivedPosCartAudit },
  ];
  ...
  for (const dataset of datasets) {
    ...
    for (let index = 0; index < dataset.rows.length; index += 500) {
      const rows = dataset.rows.slice(index, index + 500);
      await archiveRequest("/v1/archive/batch", { shiftId: String(shiftId), table: dataset.table,
        manifestTable: dataset.manifestTable, rows, hashes: rows.map(hash) });
    }
  }
