...
function createBirthdayVoucherCode(year) {
  const suffix = globalThis.crypto?.randomUUID?.().replace(/-/g, "").slice(0, 10).toUpperCase()
    || `${Date.now()}${Math.random().toString(36).slice(2, 8)}`.slice(-10).toUpperCase();
  return `BDAY${year}-${suffix}`;
}

const COMPLETED_PURCHASE_STATUSES = new Set(["paid", "closed", "completed", "complete", "delivered"]);

function isCompletedPositivePurchase(row, amountFields) {
  if (!COMPLETED_PURCHASE_STATUSES.has(String(row?.status || "").trim().toLowerCase())) return false;
  return amountFields.some((field) => Number(row?.[field] || 0) > 0);
}

export async function loyaltyMemberHasPurchase(supabase, member) {
  if (!supabase || !member?.id) return false;

  if (Number(member["Total visits"] || 0) > 0 || Number(member["Total spent"] || 0) > 0) {
    return true;
  }

  const memberId = String(member.id);
  const userId = String(member.user_id || "").trim();
  const webOrderIdentityFilter = userId
    ? `loyalty_member_id.eq.${memberId},user_id.eq.${userId}`
    : `loyalty_member_id.eq.${memberId}`;

  const [ordersResult, webOrdersResult] = await Promise.all([
    supabase
      .from("orders")
      .select("status,total,net_amount")
      .or(`loyalty_member_id.eq.${memberId},customer_id.eq.${memberId}`)
      .order("created_at", { ascending: false })
      .limit(1000),
    supabase
      .from("web_orders")
      .select("status,total,loyalty_sale_total")
      .or(webOrderIdentityFilter)
      .order("created_at", { ascending: false })
      .limit(1000),
  ]);

  if (ordersResult.error) throw ordersResult.error;
  if (webOrdersResult.error) throw webOrdersResult.error;

  return (ordersResult.data || []).some((row) => isCompletedPositivePurchase(row, ["net_amount", "total"]))
    || (webOrdersResult.data || []).some((row) => isCompletedPositivePurchase(row, ["loyalty_sale_total", "total"]));
}
...
export async function createBirthdayVoucherIfNeeded(supabase, memberId, now = new Date(), options = {}) {
  ...
  const { data: member, error: memberError } = await supabase
    ...
    .select('id,user_id,"Note","Total visits","Total spent"')
    .eq("id", memberId)
    .maybeSingle();

  if (memberError) throw memberError;
  if (!member?.id) return { created: 0, skipped: "member_not_found" };
  if (!(await loyaltyMemberHasPurchase(supabase, member))) {
    return { created: 0, skipped: "no_purchase_history" };
  }
  ...
  for (let attempt = 0; attempt < 5; attempt += 1) {
    const code = createBirthdayVoucherCode(birthdayWindow.year);
    ...
    const isCodeCollision = insertError.code === "23505"
      && String(insertError.message || "").includes("vouchers_code_key");
    if (!isCodeCollision || attempt === 4) throw insertError;
  }

  return { created: 0, skipped: "code_generation_failed" };
}
