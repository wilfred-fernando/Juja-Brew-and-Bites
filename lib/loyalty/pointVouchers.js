...
export async function createMissingPointRewardVouchers(supabase, memberId) {
  ...
  const { data: existingPointVouchers } = await supabase
    .from("vouchers")
    .select("id")
    .eq("member_id", memberId)
    .eq("reward_type", "points");
  const existingPointVoucherIds = new Set((existingPointVouchers || []).map((voucher) => String(voucher.id)));
  ...
  if (!rpcError) {
    const { data: refreshedMember } = await supabase
      ...
      .eq("id", memberId)
      .maybeSingle();
    const { data: refreshedPointVouchers } = await supabase
      .from("vouchers")
      .select("id")
      .eq("member_id", memberId)
      .eq("reward_type", "points");
    const voucherIds = (refreshedPointVouchers || [])
      .map((voucher) => String(voucher.id))
      .filter((id) => id && !existingPointVoucherIds.has(id));
    return { created: voucherIds.length, voucherIds, member: refreshedMember || activeMember };
  }
  ...
  const { data: createdVouchers, error: insertError } = await supabase
    .from("vouchers")
    .insert(rows)
    .select("id");
  ...
  return {
    created: rows.length,
    voucherIds: (createdVouchers || []).map((voucher) => voucher.id).filter(Boolean),
    member: updatedMember || activeMember,
  };
}
