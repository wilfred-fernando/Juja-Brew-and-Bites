...
export async function buildGiftCertificateDelivery(batch, certificates) {
  ...
  return {
    ...
    html: `<div style="font-family:Arial,sans-serif;line-height:1.6;max-width:680px;margin:auto">${draft.html || `<h2>Your JUJA e-Gift Certificates</h2><div style="white-space:pre-wrap">${escapeHtml(draft.text)}</div>`}${images}</div>`,
    attachments,
  };
}
