...
export function parseQuoteSummary(payload) {
  ...
  const namedPriorityFee = findNamedFee(priceBreakdown, (label) => /delivery\s*fee.*priority|priority.*delivery\s*fee|priority/i.test(label));
  const explicitPriorityFee = Number(priceBreakdown?.priorityFee || data?.priorityFee || namedPriorityFee || 0) || 0;
  const inferredPriorityFee = regularFee && total && total > regularFee ? total - regularFee : 0;
  const priorityFee = explicitPriorityFee || inferredPriorityFee || 0;
  return {
