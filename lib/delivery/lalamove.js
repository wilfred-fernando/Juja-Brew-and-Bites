...
export function getLalamoveConfig() {
  ...
  return {
    ...
    priorityFee: envNumber("LALAMOVE_PRIORITY_FEE") ?? envNumber("LALAMOVE_PRIORITY_FEE_PHP") ?? 20,
...
export function parseQuoteSummary(payload) {
  ...
  const priorityFee =
    Number(priceBreakdown?.priorityFee ?? data?.priorityFee ?? 0) ||
    0;
  return {
    quoteId: clean(data?.quotationId || data?.quoteId || data?.id),
    fee: Number(price?.amount ?? price) || null,
    regularFee: Number(priceBreakdown?.totalExcludePriorityFee ?? price?.amount ?? price) || null,
    priorityFee,
