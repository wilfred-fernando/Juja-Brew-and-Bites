...
export function webDiningOptionLabel(value) {
  ...
  if (["ONLINE: DINE-IN", "ONLINE: TAKEOUT", "ONLINE: DELIVERY"].includes(normalized)) return normalized;
  if (normalized === "DINEIN" || normalized === "DINE-IN" || normalized === "DINE IN") return "ONLINE: DINE-IN";
  if (normalized === "DELIVERY" || normalized === "ONLINE DELIVERY") return "ONLINE: DELIVERY";
...
export function normalizeKdsItems(items = []) {
  return (Array.isArray(items) ? items : []).map((item, index) => ({
    ...
    kdsReceivedAt: item.kdsReceivedAt || item.kds_received_at || item.received_at || item.added_at || null,
    kds_received_at: item.kds_received_at || item.kdsReceivedAt || item.received_at || item.added_at || null,
...
export async function appendKdsTicketItems(supabase, { sourceType, order, items = [], status = "preparing" }) {
  ...
  const appendedItems = normalizeKdsItems(items).map((item) => ({
    ...item,
    kdsReceivedAt: item.kdsReceivedAt || timestamp,
    kds_received_at: item.kds_received_at || item.kdsReceivedAt || timestamp,
  }));
  ...
  // A resumed saved ticket starts a new kitchen batch. Keep the earlier items in
  // the ticket history, but retire them from the live KDS view so the kitchen
  // receives only the newly appended items.
  const retiredExistingItems = existingItems.map((item) => ({
    ...item,
    ready: true,
    kitchenReady: true,
    kitchen_ready: true,
    ready_at: item.kitchenReadyAt || item.ready_at || timestamp,
    kitchenReadyAt: item.kitchenReadyAt || item.ready_at || timestamp,
    kitchen_ready_at: item.kitchenReadyAt || item.ready_at || timestamp,
    kdsCompleted: true,
    kds_completed: true,
...
export async function markKdsTicketItemVoided(supabase, { sourceType, sourceId, itemId, itemName }) {
  if (!supabase || !sourceType || !sourceId || (!itemId && !itemName)) return { data: null, error: null, matched: false };
  ...
  if (error || !ticket) return { data: ticket, error, matched: false };
  ...
  if (!matched) return { data: ticket, error: null, matched: false };
