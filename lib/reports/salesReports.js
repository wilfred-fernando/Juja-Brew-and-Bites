...
function importedStoreId(row, storeByName) {
  const name = firstText(row, ["store_name", "Store"], "Imported store");
  const store = storeByName.get(name.toLowerCase());
  return {
    storeId: store?.id ? String(store.id) : `import:${name}`,
    storeName: store?.name || name,
  };
}

function importedReceiptStatus(row) {
  const type = firstText(row, ["receipt_type", "Receipt type"]).toLowerCase();
  if (type.includes("refund")) return "Refunded";
  if (type.includes("void")) return "Voided";
  return isCompletedStatus(row.status || row.Status) ? "Paid" : firstText(row, ["status", "Status"], "Paid");
}

function importedReceiptAmounts(row) {
  if (row?.refund_amount !== undefined) {
    return {
      gross: firstNumber(row, ["gross_sales", "Gross sales"], 0),
      discount: Math.abs(firstNumber(row, ["discounts", "Discounts"], 0)),
      refund: Math.abs(firstNumber(row, ["refund_amount"], 0)),
      net: firstNumber(row, ["net_sales", "Net sales"], 0),
    };
  }
  const type = firstText(row, ["receipt_type", "Receipt type"]).toLowerCase();
  const grossValue = firstNumber(row, ["gross_sales", "Gross sales"], 0);
  const discountValue = Math.abs(firstNumber(row, ["discounts", "Discounts"], 0));
  const netValue = firstNumber(row, ["net_sales", "Net sales", "total_collected", "Total collected"], 0);
  const isRefund = type.includes("refund") || netValue < 0 || grossValue < 0;
  if (isRefund) {
    return {
      gross: 0,
      discount: 0,
      refund: Math.abs(netValue || grossValue),
      net: 0,
    };
  }
  return {
    gross: Math.max(0, grossValue),
    discount: discountValue,
    refund: 0,
    net: Math.max(0, netValue),
  };
}

export function normalizeSalesData({ orders = [], orderItems = [], webOrders = [], menuItems = [], profiles = [], stores = [], shiftRecords = [], importedReceipts = [], importedReceiptItems = [] }) {
  ...
  const storeByName = new Map(stores.map((store) => [String(store.name || store.store_name || "").toLowerCase(), store]));
  ...
  const sales = [
    ...
    ...importedReceipts.map((row) => {
      const { storeId, storeName } = importedStoreId(row, storeByName);
      const amounts = importedReceiptAmounts(row);
      const receipt = firstText(row, ["receipt_number", "Receipt number"], String(row.id || ""));
      return {
        id: `imported-receipt:${row.id || row.row_hash || receipt}`,
        orderKey: `imported:${storeId}:${receipt}`,
        source: "Imported",
        createdAt: row.receipt_date,
        date: row.receipt_day || businessDate(row.receipt_date, storeId, shiftRecords),
        orderNumber: receipt,
        customerName: firstText(row, ["customer_name", "Customer name"], "Walk-in"),
        orderType: orderTypeLabel({ dining_option: firstText(row, ["dining_option", "Dining option"]) }, "POS"),
        paymentMethod: paymentLabel(row.payment_type || row["Payment type"]),
        ...amounts,
        cashierId: firstText(row, ["cashier_name", "Cashier name"], "IMPORTED"),
        cashierName: firstText(row, ["cashier_name", "Cashier name"], "Imported cashier"),
        storeId,
        storeName,
        status: importedReceiptStatus(row),
        orderCount: firstNumber(row, ["order_count"], 1),
        raw: row,
      };
    }),
  ];
  ...
  importedReceiptItems.forEach((item) => {
    const { storeId, storeName } = importedStoreId(item, storeByName);
    const receipt = firstText(item, ["receipt_number", "Receipt number"], String(item.id || ""));
    const qty = firstNumber(item, ["quantity", "Quantity"], 0);
    const net = firstNumber(item, ["net_sales", "Net sales"], 0);
    const gross = firstNumber(item, ["gross_sales", "Gross sales"], net);
    const discount = Math.abs(firstNumber(item, ["discounts", "Discounts"], 0));
    const itemName = firstText(item, ["item", "Item"], "Item");
    const variant = firstText(item, ["variant", "Variant"]);
    lineItems.push({
      id: `imported-item:${item.id || item.row_hash || `${receipt}-${itemName}-${variant}`}`,
      orderId: `imported:${storeId}:${receipt}`,
      source: "Imported",
      date: item.receipt_day || businessDate(item.receipt_date, storeId, shiftRecords),
      orderNumber: receipt,
      itemId: firstText(item, ["sku", "SKU"], `${itemName}${variant ? ` (${variant})` : ""}`),
      itemName: variant ? `${itemName} (${variant})` : itemName,
      category: firstText(item, ["category", "Category"], "Uncategorized"),
      quantity: qty,
      unitPrice: qty ? net / qty : net,
      gross,
      discount,
      net,
      orderType: orderTypeLabel({ dining_option: firstText(item, ["dining_option", "Dining option"]) }, "POS"),
      storeId,
      storeName,
      status: importedReceiptStatus(item),
    });
  });
...
export function getSalesSummary(rows) {
  const summary = rows.reduce(
    (acc, row) => ({
      ...
      net: acc.net + row.net,
      orders: acc.orders + (row.status === "Paid" ? (row.orderCount || 1) : 0),
...
export function getPaymentReport(rows, totalNet = 0) {
  ...
  rows.forEach((row) => {
    ...
    current.transactions += row.status === "Paid" ? (row.orderCount || 1) : 0;
...
export function getCashierReport(rows) {
  ...
  rows.forEach((row) => {
    ...
    current.orders += row.status === "Paid" ? (row.orderCount || 1) : 0;
    current.gross += row.gross;
    current.discounts += row.discount;
    current.refunds += row.refund;
    current.net += row.net;
