...
import { shiftBusinessDate } from "@/lib/businessDay";
...
export function rangeFromPreset(preset) {
  ...
  if (preset === "last_7_days") return { startDate: addDays(today, -6), endDate: today };
  if (preset === "last_30_days") return { startDate: addDays(today, -29), endDate: today };
...
function isSaleStatus(status) {
  return ["Paid", "Partial Refund"].includes(String(status || ""));
}
...
function isPartialRefund(row) {
  const status = `${row?.status || ""} ${row?.payment_status || ""}`.toLowerCase();
  const total = Math.abs(firstNumber(row, ["net_amount", "total", "net_sales", "subtotal"], 0));
  const refund = Math.abs(firstNumber(row, ["refund_amount", "refund", "refunds"], 0));
  return status.includes("partial_refund") || status.includes("partial refund") || (refund > 0 && (!total || refund < total));
}

function isFullRefundOrVoid(row) {
  const status = `${row?.status || ""} ${row?.payment_status || ""}`.toLowerCase();
  if (isPartialRefund(row)) return false;
  return Boolean(row?.voided_at || status.includes("void") || status.includes("refund") || row?.refunded_at);
}
...
function salesPaymentSplitEntries(rawSplits, paymentMethod, fallbackAmount = 0) {
  const fromJson = Array.isArray(rawSplits)
    ? rawSplits
        .map((entry) => ({
          method: normalizedPaymentMethod(entry.method || entry.payment_method || entry.type || entry.name),
  ...
  const label = String(paymentMethod || "").trim();
  if (!label) return [];
  ...
  if (parts.length > 1) {
    const parsed = parts
      .map((part) => {
        ...
        return match ? { method: normalizedPaymentMethod(match[1]), amount: parsePaymentAmountText(match[2]) } : null;
      })
      .filter((entry) => entry?.method && entry.amount > 0);
    if (parsed.length > 0) return parsed;
  }
  return [{ method: normalizedPaymentMethod(label), amount: Number(fallbackAmount || 0) }];
}
...
function businessDate(value, storeId, shiftRecords = []) {
  if (!value) return manilaDate();
  return shiftBusinessDate(value, storeId, shiftRecords);
}
...
export function deduplicateReportRows(rows = []) {
  const byId = new Map();
  for (const row of rows) {
    // The report loader supplies archive rows first, followed by live rows.
    // Replace the full archived record with its live copy, including refunds.
    // Rows without IDs remain distinct; product names are not row identities.
...
export function normalizeSalesData({ orders = [], orderItems = [], webOrders = [], menuItems = [], profiles = [], stores = [], shiftRecords = [], loyaltyMembers = [] }) {
  orders = deduplicateReportRows(orders);
  orderItems = deduplicateReportRows(orderItems);
  webOrders = deduplicateReportRows(webOrders);
  shiftRecords = deduplicateReportRows(shiftRecords);
  const menuById = new Map(menuItems.map((item) => [String(item.id), item]));
  const webOrderById = new Map(webOrders.map((order) => [String(order.id), order]));
  const webOrderByReceipt = new Map(
    webOrders
      .filter((order) => firstText(order, ["receipt_number", "order_number"]))
      .map((order) => [firstText(order, ["receipt_number", "order_number"]), order])
  );
  const convertedWebOrderIds = new Set(
    orders.map((order) => firstText(order, ["source_web_order_id"])).filter(Boolean)
  );
  const posReceiptNumbers = new Set(
    orders.map((order) => firstText(order, ["receipt_number", "order_number"])).filter(Boolean)
  );
  const standaloneCompletedWebOrders = webOrders.filter((order) => {
    if (!isCompletedStatus(order.status)) return false;
    if (convertedWebOrderIds.has(String(order.id))) return false;
    const receipt = firstText(order, ["receipt_number", "order_number"]);
    return !receipt || !posReceiptNumbers.has(receipt);
  });
  ...
  const loyaltyById = new Map(loyaltyMembers.map((member) => [String(member.id), member]));
  ...
  const normalizeOrder = (row, source) => {
    ...
    const refund = firstNumber(row, ["refund_amount", "refund", "refunds"], isFullRefundOrVoid(row) ? total : 0);
    ...
    const linkedCustomerId = firstText(row, ["loyalty_member_id", "customer_id"]);
    const linkedCustomer = loyaltyById.get(linkedCustomerId);
    const linkedCustomerName = firstText(linkedCustomer, ["customer_name", "name", "full_name"]);
    const linkedCustomerCode = firstText(linkedCustomer, ["customer_code", "Customer ID", "code"]);
    const linkedCustomerPhone = firstText(linkedCustomer, ["Phone", "phone", "contact_number", "customer_contact"]);
    const linkedAvailablePoints = firstNumber(linkedCustomer, ["Available points", "available_points"], null);
    const linkedPointsBalance = firstNumber(linkedCustomer, ["Points balance", "points_balance"], null);
    const status = row?.voided_at
      ? "Voided"
      : isPartialRefund(row)
        ? "Partial Refund"
        : isFullRefundOrVoid(row)
          ? "Refunded"
          : isCompletedStatus(row?.status)
            ? "Paid"
            : (row?.status || "Paid");
    const rawPaymentMethod = row.payment_method || row.payment_type;
    const paymentMethod = paymentLabel(rawPaymentMethod);
    const paymentSplits = salesPaymentSplitEntries(row?.source_metadata?.payment_splits, rawPaymentMethod, net);
    const linkedWebOrder = source === "POS"
      ? webOrderById.get(firstText(row, ["source_web_order_id"]))
        || webOrderByReceipt.get(receiptNumber(row, source))
      : null;
    const createdAt = linkedWebOrder?.created_at || row.paid_at || row.created_at;
    const salesAt = row.paid_at || row.completed_at || row.created_at;
    const refundedAt = [row.refunded_at, ...(source === "POS" ? itemsByOrder.get(String(row.id)) || [] : []).map((item) => item.refunded_at)]
    ...
    return {
      ...
      salesAt,
      refundedAt,
      receiptAt: refundedAt || createdAt,
      date: businessDate(salesAt, storeId, shiftRecords),
      orderNumber: receiptNumber(row, source),
      customerName: linkedCustomerName || firstText(row, ["customer_name", "customer"], source === "Web" ? "Web customer" : "Walk-in"),
      customerCode: linkedCustomerCode || firstText(row, ["customer_code"]),
      customerPhone: linkedCustomerPhone || firstText(row, ["customer_phone", "phone", "contact_number"]),
      customerAvailablePoints: linkedAvailablePoints,
      customerPointsBalance: linkedPointsBalance,
      ...
      refund,
      net,
      loyaltyPointsAwarded: firstNumber(row, ["loyalty_points_awarded", "points_earned", "loyalty_points"], null),
  ...
  const sales = [
    ...
    ...standaloneCompletedWebOrders.map((order) => normalizeOrder(order, "Web")),
  ];

  const lineItems = [];
  orders.forEach((order) => {
    const normalized = sales.find((row) => row.id === order.id && row.source === "POS");
    enrichReceiptItemRows(itemsByOrder.get(String(order.id)) || [], order.items).forEach((item) => {
      ...
      const itemNet = firstNumber(item, ["net_amount", "line_total"], gross - discount);
      const parentRefunded = normalized?.status === "Refunded" || normalized?.status === "Voided";
      const itemRefund = firstNumber(item, ["refund_amount"], isRefundOrVoid(item) || parentRefunded ? itemNet : 0);
      const itemStatus = isRefundOrVoid(item) || parentRefunded ? "Refunded" : normalized?.status;
      const itemNetAfterRefund = Math.max(0, itemNet - itemRefund);
      lineItems.push({
        id: item.id,
        orderId: order.id,
        source: "POS",
        ...
        quantity: itemNetAfterRefund > 0 ? qty : 0,
        unitPrice: firstNumber(item, ["unit_price"], qty ? itemNet / qty : itemNet),
        gross,
        discount,
        net: itemNetAfterRefund,
        refund: itemRefund,
  ...
  standaloneCompletedWebOrders.forEach((order) => {
  ...
  return { sales, lineItems, shiftRecords };
}
...
export function applyFilters({ sales, lineItems }, filters) {
  ...
  const orderIds = new Set(salesRows.map((row) => String(row.orderKey || row.id)));
...
export function getSalesSummary(rows) {
  const summary = rows.reduce(
    (acc, row) => {
      const key = paymentKey(row.paymentMethod);
      return {
        gross: acc.gross + row.gross,
        discount: acc.discount + row.discount,
        refund: acc.refund + row.refund,
        net: acc.net + row.net,
        orders: acc.orders + (isSaleStatus(row.status) ? (row.orderCount || 1) : 0),
        cash: acc.cash + (key === "cash" ? row.net : 0),
        gcash: acc.gcash + (key === "gcash" ? row.net : 0),
        qrph: acc.qrph + (key === "qrph" ? row.net : 0),
        card: acc.card + (key === "card" ? row.net : 0),
        online: acc.online + (["grabfood", "foodpanda", "panda", "online", "grabdineout"].includes(key) || row.source === "Web" ? row.net : 0),
      };
    },
    { gross: 0, discount: 0, refund: 0, net: 0, orders: 0, cash: 0, gcash: 0, qrph: 0, card: 0, online: 0 }
  );
...
export function getSalesTrend(rows, startDate, endDate) {
  ...
  rows.forEach((row) => {
    ...
    current.net += row.net;
    current.orders += row.orderCount || 1;
...
export function getHourlySales(rows) {
  ...
  rows.forEach((row) => {
    const hour = Number(new Intl.DateTimeFormat("en-US", { timeZone: MANILA_TZ, hour: "numeric", hour12: false }).format(new Date(row.salesAt || row.createdAt)));
...
export function getPaymentReport(rows, totalNet = 0) {
  const map = new Map();
  rows.forEach((row) => {
    const splits = Array.isArray(row.paymentSplits) && row.paymentSplits.length > 0
      ? row.paymentSplits
      : salesPaymentSplitEntries(null, row.paymentMethod, row.net);
    const splitTotal = splits.reduce((sum, split) => sum + Number(split.amount || 0), 0) || row.net || 1;
    splits.forEach((split) => {
      const key = split.method || "Other";
      const ratio = splitTotal ? Number(split.amount || 0) / splitTotal : 1 / splits.length;
      const current = map.get(key) || { paymentMethod: key, transactions: 0, gross: 0, discounts: 0, refunds: 0, net: 0, refundTransactions: 0 };
      current.transactions += isSaleStatus(row.status) ? (row.orderCount || 1) : 0;
      current.refundTransactions += row.status === "Refunded" || row.status === "Voided" ? (row.orderCount || 1) : 0;
      current.gross += row.gross * ratio;
      current.discounts += row.discount * ratio;
      current.refunds += row.refund * ratio;
      current.net += row.net * ratio;
      map.set(key, current);
    });
  });
...
export function getCashierReport(rows) {
  ...
  rows.forEach((row) => {
    ...
    current.orders += isSaleStatus(row.status) ? (row.orderCount || 1) : 0;
    current.gross += row.gross;
    ...
    splits.forEach((split) => {
      const key = split.method || "Other";
      const ratio = splitTotal ? Number(split.amount || 0) / splitTotal : 1 / splits.length;
      current.payments[key] = (current.payments[key] || 0) + row.net * ratio;
    });
    map.set(key, current);
  });
  return Array.from(map.values())
...
export function getVoidRefundReport(rows) {
  return rows
    ...
    .map((row) => ({
      dateTime: displayDateTime(row.refundedAt || row.raw?.voided_at || row.createdAt),
