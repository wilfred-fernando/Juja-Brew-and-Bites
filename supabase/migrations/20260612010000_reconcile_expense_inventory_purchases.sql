-- Backfill and repair inventory stock from existing mapped expense purchases.
-- Safe/additive migration: no production rows are deleted, truncated, or renamed.
-- Rerunnable: expense_inventory_links prevents duplicate purchase movements.
...
declare
  ...
  mapped_movement_type text := 'purchase';
begin
...
revoke all on function public.reconcile_expense_inventory_purchases() from anon;
grant execute on function public.reconcile_expense_inventory_purchases() to authenticated;
