...
export default function POSPage() {
  ...
  const handleSaveTicket = async () => {
    ...
    const label = prompt("Enter Ticket Label (e.g. Table 5):");
    if (!label) return;
  ...
  return (
    <div className="flex h-screen bg-white font-sans overflow-hidden text-slate-800">
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex-1 overflow-y-auto p-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 auto-rows-max hide-scrollbar">
      ...
      <div className="w-[340px] bg-white border-l border-slate-100 flex flex-col">
        <div className="p-4 border-b border-slate-50 flex-shrink-0">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-xs font-semibold text-slate-400 uppercase tracking-widest">New Ticket</h2>
            <div className="flex gap-2">
              <button onClick={() => setConfirmClear(true)} className="text-slate-300 hover:text-rose-500 text-xl transition-colors">✕</button>
              <button onClick={handleSaveTicket} className="w-8 h-8 flex items-center justify-center bg-slate-50 rounded-lg text-sm hover:bg-slate-100 transition-colors" title="Save Ticket">📥</button>
            </div>
          </div>
