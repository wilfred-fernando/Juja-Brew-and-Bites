      ...
      <div className="bg-white w-full max-w-md rounded-t-3xl md:rounded-3xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-6 hide-scrollbar">
          ...
          <div className="space-y-2">
            <label className="text-[11px] text-slate-400 uppercase tracking-wider">Instructions</label>
...
// ─── 2. MODAL: CUSTOM ALERT (REPLACES DEFAULT OK/CANCEL) ───
  ...
  return (
    <div className="fixed inset-0 z-[500] bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-6 animate-in fade-in duration-200">
      <div className="bg-white w-full max-w-sm rounded-[32px] p-8 shadow-2xl text-center animate-in zoom-in-95 duration-300">
        <div className={`w-16 h-16 rounded-full flex items-center justify-center text-2xl mx-auto mb-4 ${type === 'danger' ? 'bg-rose-50 text-rose-500' : 'bg-slate-50 text-slate-500'}`}>
          {type === 'danger' ? '✕' : '?'}
        </div>
        ...
        <div className="flex gap-3">
          ...
          <button onClick={onConfirm} className={`flex-[2] py-4 rounded-2xl text-sm font-medium text-white transition-all active:scale-95 shadow-lg ${type === 'danger' ? 'bg-rose-500 shadow-rose-100' : 'bg-slate-800'}`}>Confirm</button>
        </div>
      </div>
    </div>
  );
...
export default function POSPage() {
  ...
  async function fetchData() {
    setLoading(true);
    const [iRes, catRes, cRes, dRes] = await Promise.all([
    ...
    <div className="flex h-screen bg-white font-sans overflow-hidden text-slate-800">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="p-4 border-b border-slate-50 flex items-center justify-between gap-4">
            ...
            <div className="flex gap-2 flex-1 md:flex-none justify-end">
               <select value={activeCategory} onChange={(e) => setActiveCategory(e.target.value)} className="bg-slate-50 px-3 py-2 rounded-lg text-xs outline-none cursor-pointer hover:bg-slate-100 transition-colors">
      ...
      <div className={`${mobileCartOpen ? 'translate-y-0' : 'translate-y-full lg:translate-y-0'} fixed lg:relative bottom-0 left-0 right-0 h-[90vh] lg:h-full lg:w-[340px] bg-white border-l border-slate-100 flex flex-col z-[300] transition-transform duration-300 rounded-t-3xl lg:rounded-none shadow-2xl lg:shadow-none`}>
        <div className="p-4 border-b border-slate-50 flex-shrink-0">
           ...
           <div className="space-y-2">
             <form onSubmit={handleScanSubmit} className="flex gap-2 group">
                ...
                <input id="scan-in" type="text" placeholder="Scan loyalty/product..." value={customerSearch} onFocus={() => setIsCustListOpen(true)} onChange={(e) => setCustomerSearch(e.target.value)} className="flex-1 px-3 py-2 bg-slate-50 border-none rounded-lg text-sm outline-none focus:bg-slate-100" />
             </form>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3 hide-scrollbar">
           {cart.length === 0 ? (
             <div className="h-full flex items-center justify-center opacity-10 text-[10px] uppercase font-semibold">Ready for orders</div>
           ) : (
             cart.map((item, idx) => (
               <div key={item.cartItemId} className="flex justify-between items-start border-b border-slate-50 pb-2">
                  <div className="flex-1 pr-3">
                    ...
                    {item.instructions && <p className="text-[10px] text-amber-600 mt-1 italic leading-tight">"{item.instructions}"</p>}
                    <button onClick={() => { const n = [...cart]; n.splice(idx,1); setCart(n); }} className="text-[10px] text-slate-300 hover:text-red-500 mt-1.5 transition-colors underline underline-offset-2">Remove</button>
                  </div>
      ...
      {confirmClear && (
        <ConfirmModal 
          ...
          onConfirm={() => { setCart([]); setAttachedCustomer(null); setConfirmClear(false); }} 
