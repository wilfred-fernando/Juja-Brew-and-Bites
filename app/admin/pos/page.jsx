...
export default function POS() {
  ...
  useEffect(() => {
    if (isScannerOpen) {
      if (localStorage.getItem('juja_cam_granted')) startCamera(); 
  ...
  const place = (isPaid = true, paymentMethod = "Unpaid") => {
    ...
    Order.create({
      ...
      clear(); 
    }).catch((err) => alert("Order failed. Details: " + (err.message || JSON.stringify(err))))
  ...
  return (
    // UPDATED CONTAINER: Accounts for layout padding dynamically (calc 100dvh - 3rem) and adds beautiful rounded corners!
    <div className="h-[100dvh] md:h-[calc(100dvh-3rem)] w-full flex flex-col lg:flex-row overflow-hidden bg-white md:rounded-[32px] md:shadow-xl md:border border-rose-100 animate-in fade-in duration-500">
      
      {/* ─── LEFT PANEL: MENU ─── */}
      <div className="flex-1 flex flex-col min-w-0 relative h-full bg-white">
        
        <div className="flex-shrink-0 flex items-center justify-between px-4 py-4 md:px-6 bg-white border-b border-slate-100 shadow-[0_2px_10px_rgba(0,0,0,0.02)] z-10">
          <div className="flex items-center gap-3 flex-1">
            <button className="p-2 -ml-2 text-slate-500 hover:bg-slate-50 rounded-lg transition-colors md:hidden">
            ...
            <select value={cat} onChange={e=>setCat(e.target.value)} className="bg-transparent font-black text-slate-800 text-lg md:text-xl focus:outline-none cursor-pointer max-w-[150px] md:max-w-xs truncate">
        ...
        {/* UPDATED GRID: Forces 1 column on cramped screens to prevent overlap, expands to 2 only when safe! */}
        <div className="flex-1 overflow-y-auto hide-scrollbar pb-24 lg:pb-0 bg-slate-50/30">
          <div className="grid grid-cols-1 xl:grid-cols-2 divide-y xl:divide-none divide-slate-100 p-0 xl:p-4 xl:gap-3">
            {filtered.map(item=>{
              const qty = getQty(item.id);
              return (
                <button key={item.id} onClick={()=>add(item)} 
                  className="flex items-center p-4 md:p-5 xl:rounded-2xl xl:border border-slate-100 hover:bg-slate-50 active:bg-slate-100 transition-all text-left w-full relative overflow-hidden group bg-white">
                  
                  {/* Image - Protected from squishing via shrink-0 */}
                  <div className="w-12 h-12 md:w-16 md:h-16 shrink-0 bg-[#FFF5F7] rounded-xl border border-rose-50 flex items-center justify-center relative shadow-sm">
                    {item.image_url ? (
                    ...
                    {qty>0 && (
                      <div className="absolute -top-2 -right-2 w-6 h-6 md:w-7 md:h-7 rounded-full bg-[#FC687D] flex items-center justify-center text-xs md:text-sm font-black text-white shadow-md animate-in zoom-in">
                        {qty}
                      </div>
                    )}
                  </div>
                  
                  {/* Text Container - Protected from pushing right side via min-w-0 & truncate */}
                  <div className="flex-1 min-w-0 px-4 flex flex-col justify-center">
                    <p className="text-slate-800 font-bold text-sm md:text-base truncate leading-tight">{item.name}</p>
                    <p className="text-slate-400 text-[10px] md:text-xs font-bold uppercase tracking-widest mt-1 truncate">{item.category}</p>
                  </div>
                  
                  {/* Price - Protected from squishing via shrink-0 */}
                  <div className="shrink-0 font-black text-slate-800 text-sm md:text-lg">
      ...
      <div className={`fixed inset-0 z-[150] lg:static lg:z-auto w-full lg:w-[360px] xl:w-[420px] flex-shrink-0 flex flex-col bg-slate-50 lg:border-l border-slate-200 shadow-2xl lg:shadow-none transition-transform duration-300 ${showMobileTicket ? "translate-y-0" : "translate-y-full lg:translate-y-0"}`}>

        <div className="flex-shrink-0 flex items-center justify-between px-4 py-4 md:px-6 bg-white border-b border-slate-200 pt-safe z-10">
        ...
        <div className="flex-shrink-0 flex flex-col p-4 md:p-5 border-b border-slate-200 bg-white">
          <div className="relative mb-3">
            <button onClick={() => setIsScannerOpen(true)} title="Open Camera" className="absolute left-1.5 top-1/2 -translate-y-1/2 p-1.5 text-slate-500 hover:text-white hover:bg-[#FC687D] bg-slate-100 border border-slate-200 rounded-md transition-all z-10 active:scale-90 flex items-center justify-center shadow-sm">
            ...
            <input 
              ...
              onKeyDown={e => {
                if (e.key === 'Enter') {
                  ...
                  if (exactMatch) { setCustSearch(exactMatch["Customer name"]); setCname(exactMatch["Customer name"]); setShowCustList(false); }
                }
              }}
          ...
          <div className="flex gap-2">
            <div className="relative flex-1">
              <select value={orderType} onChange={e=>setOrderType(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-lg pl-3 pr-8 py-2.5 text-xs font-bold text-slate-700 uppercase tracking-widest focus:outline-none focus:border-[#FC687D] transition-all appearance-none cursor-pointer">
            ...
            {(orderType === "VIP Room" || orderType === "Table") && (
              <input value={tableNum} onChange={e=>setTableNum(e.target.value)} placeholder={`#`} className="w-16 bg-slate-50 border border-slate-200 rounded-lg px-2 py-2.5 text-xs font-bold focus:outline-none focus:border-[#FC687D] transition-all text-center" />
            )}
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto hide-scrollbar bg-slate-50/50">
          {cart.length === 0 ? (
            ...
            <div className="divide-y divide-slate-200/50">
              {cart.map(item => (
                <div key={item.id} className="flex items-center p-4 bg-transparent hover:bg-slate-100 transition-colors">
        ...
        <div className="flex-shrink-0 bg-slate-50 border-t border-slate-200">
          ...
          {showDisc && (
            <div className="flex gap-2 px-4 lg:px-5 pb-4">
              {[0,5,10,15,20].map(d=>(
                <button key={d} onClick={()=>{setDisc(d);setShowDisc(false);}} className={`flex-1 py-2.5 rounded-lg font-bold text-xs transition-all active:scale-95 border ${disc===d ? "bg-[#FC687D] text-white border-[#FC687D] shadow-md" : "bg-white text-slate-600 border-slate-300"}`}>
                  {d===0?"None":d+"%"}
                </button>
              ))}
            </div>
        ...
        <div className="flex-shrink-0 bg-white p-3 md:p-4 border-t border-slate-200 pb-safe">
          <div className="flex gap-2">
            <button onClick={() => place(false, "Unpaid")} disabled={!cart.length||busy} className="flex-1 py-4 rounded-xl font-black text-xs uppercase tracking-widest text-[#10b981] bg-[#10b981]/10 hover:bg-[#10b981]/20 transition-all active:scale-95 disabled:opacity-50 disabled:active:scale-100 border border-[#10b981]/20">
              SAVE
            </button>
            <button onClick={handleChargeClick} disabled={!cart.length||busy} className="flex-[1.5] py-4 rounded-xl font-black text-sm text-white transition-all active:scale-95 flex flex-col items-center justify-center shadow-md disabled:opacity-50 disabled:shadow-none" style={{ backgroundColor: cart.length ? "#10b981" : "#cbd5e1" }}>
        ...
        <div className="fixed inset-0 z-[400] bg-slate-900/90 backdrop-blur-sm flex flex-col items-center justify-center p-4 animate-in fade-in">
          <div className="bg-white rounded-3xl w-full max-w-md overflow-hidden shadow-2xl relative flex flex-col min-h-[400px]">
            <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              <h3 className="font-black text-slate-800 flex items-center gap-2"><svg className="w-5 h-5 text-[#FC687D]" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M3 5h2v14H3zM7 5h1v14H7zM10 5h2v14h-2zM14 5h1v14h-1zM17 5h2v14h-2zM20 5h1v14h-1z" /></svg>Scan Loyalty Card</h3>
            ...
            <div className="relative bg-black/5 flex-1 flex flex-col p-4 overflow-hidden">
              ...
              {cameraStatus === "idle" && (
                <div className="absolute inset-0 bg-white flex flex-col items-center justify-center p-8 text-center z-10">
                  ...
                  <button onClick={startCamera} className="w-full py-4 bg-[#FC687D] text-white font-black text-xs uppercase tracking-widest rounded-xl shadow-md active:scale-95 transition-all">Enable Camera</button>
                </div>
              )}
              ...
              {cameraStatus === "error" && (
                <div className="absolute inset-0 bg-white flex flex-col items-center justify-center p-8 text-center z-10">
                  ...
                  <button onClick={startCamera} className="w-full py-4 bg-slate-800 text-white font-black text-xs uppercase tracking-widest rounded-xl shadow-md active:scale-95 transition-all">Try Again</button>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-[200] bg-slate-900/80 backdrop-blur-sm flex flex-col pt-10 pb-safe px-4 animate-in fade-in duration-200">
          <div className="w-full max-w-md mx-auto bg-[#1a1a1a] rounded-3xl overflow-hidden flex flex-col flex-1 max-h-[800px] shadow-2xl border border-slate-800">
            <div className="flex items-center px-4 py-4 border-b border-slate-800 relative">
              <button onClick={() => setShowPaymentModal(false)} className="text-white p-2"><svg width="24" height="24" fill="none" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg></button>
            ...
            <div className="px-6 py-6">
              ...
              <input type="number" step="0.01" value={amountTendered} onChange={e=>setAmountTendered(e.target.value)} className="w-full bg-transparent border-b border-slate-600 text-white font-black text-xl py-2 focus:outline-none focus:border-emerald-500 transition-colors"/>
            </div>
            <div className="flex-1 overflow-y-auto px-6 pb-6 space-y-3 hide-scrollbar">
              {[{ id: "CASH", icon: "💵" }, { id: "GRABFOOD", icon: "🥡" }, { id: "QRPH", icon: "📱" }, { id: "GRAB DINE OUT", icon: "🍽️" }, { id: "CARD", icon: "💳" }].map(pm => (
                <button key={pm.id} onClick={() => { setShowPaymentModal(false); place(true, pm.id); }} className="w-full py-4 bg-[#2a2a2a] hover:bg-[#333] active:bg-[#444] rounded-xl flex items-center justify-center gap-3 transition-colors border border-slate-700/50">
                  <span className="text-lg grayscale opacity-70">{pm.icon}</span><span className="font-bold text-sm text-white tracking-wide">{pm.id}</span>
                </button>
              ))}
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-[300] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-300">
          <div className="bg-white rounded-[24px] w-full max-w-sm overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300">
            <div className="p-6 text-center border-b border-slate-100 bg-[#FFF9FA]">
              ...
              <h2 className="font-black text-xl text-slate-800 mb-1">{receipt.isPaid ? "Transaction Complete" : "Ticket Saved!"}</h2>
            ...
            <div className="p-5 max-h-[40vh] overflow-y-auto hide-scrollbar bg-slate-50">
              ...
              <div className="border-t border-slate-200 pt-4 mt-4 space-y-2">
                <div className="flex justify-between font-black text-lg text-slate-800 pt-1"><span>{receipt.isPaid ? "Total Paid" : "Balance Due"}</span><span>₱{formatMoney(receipt.total)}</span></div>
                {receipt.isPaid && receipt.method === "CASH" && (<div className="flex justify-between text-xs font-bold text-emerald-500"><span>Change</span><span>₱{formatMoney(receipt.change)}</span></div>)}
              </div>
            ...
            <div className="flex flex-col gap-2 p-5 bg-white">
              <button onClick={()=>window.print()} className="w-full py-3 rounded-xl border border-slate-200 bg-white font-bold text-slate-600 text-xs hover:bg-slate-50 active:scale-95 transition-all">Print Ticket</button>
              <button onClick={()=>setReceipt(null)} className="w-full py-3 rounded-xl border-none bg-[#FC687D] text-white font-bold text-xs shadow-md hover:bg-rose-500 active:scale-95 transition-all">New Order</button>
            </div>
          </div>
        </div>
      )}
