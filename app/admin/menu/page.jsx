  ...
  return (
    ...
    >
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 md:gap-6 mb-4 md:mb-8 pt-4 md:pt-6">
        <div>
          <h1 className="text-2xl md:text-4xl font-black text-slate-800 tracking-tight leading-none">Menu Builder</h1>
        ...
        <div className="flex w-full md:w-auto gap-2 md:gap-3">
          <button onClick={() => openModal()} className="flex-1 md:flex-none px-4 md:px-6 py-2.5 md:py-3.5 bg-[#FC687D] text-white text-[11px] md:text-sm font-black uppercase tracking-widest rounded-xl md:rounded-2xl hover:bg-rose-500 transition-all shadow-[0_4px_15px_rgba(252,104,125,0.25)] hover:-translate-y-0.5 active:scale-95">
            + Add Item
          </button>
          <button className="flex-1 md:flex-none px-4 md:px-6 py-2.5 md:py-3.5 bg-white border border-rose-100 text-[#FC687D] text-[11px] md:text-sm font-black uppercase tracking-widest rounded-xl md:rounded-2xl hover:bg-rose-50 transition-all shadow-sm active:scale-95">
            + Category
          </button>
        </div>
      </header>
      ...
      <div className="flex flex-col lg:flex-row gap-4 md:gap-8">
        ...
        <div className="w-full lg:w-72 flex-shrink-0">
          ...
          <div className="flex lg:flex-col overflow-x-auto lg:overflow-visible hide-scrollbar gap-2 lg:gap-1 pb-2 lg:pb-0 -mx-3 px-3 lg:mx-0 lg:px-0 lg:bg-white lg:p-2 lg:rounded-2xl lg:border lg:border-slate-100 lg:shadow-sm">
            <h3 className="hidden lg:block text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 px-3 pt-2 pb-2">Categories</h3>
            
            <button 
              onClick={() => setCatFilter("All")}
              className={`flex-shrink-0 lg:w-full flex items-center justify-between px-4 lg:px-3 py-2.5 lg:py-2.5 rounded-xl text-[10px] lg:text-xs font-black uppercase tracking-widest transition-all duration-300 active:scale-95 ${
          ...
          {filteredItems.map(item => (
            <div key={item.id} className="bg-white rounded-xl md:rounded-2xl border border-rose-50 shadow-sm p-3 md:p-4 pr-3 md:pr-5 flex flex-col md:flex-row md:items-center justify-between gap-3 md:gap-0 hover:shadow-md transition-all duration-300 group cursor-default">
              ...
              <div className="flex items-center gap-3 md:gap-4">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.1em] text-slate-400">{item.category}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between md:justify-end w-full md:w-auto gap-3 md:gap-6 pt-3 md:pt-0 border-t md:border-none border-slate-50">
                <span className={`px-2 md:px-3 py-1 md:py-1.5 rounded-lg text-[9px] md:text-[10px] font-black uppercase tracking-widest border flex items-center gap-1.5 ${
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-sm flex items-end md:items-center justify-center p-0 md:p-4 transition-all duration-300" onClick={() => setIsModalOpen(false)}>
          <div className="bg-white w-full max-w-2xl rounded-t-[20px] md:rounded-[24px] p-5 md:p-8 shadow-2xl animate-in slide-in-from-bottom-full md:slide-in-from-bottom-10 md:zoom-in-95 duration-300 max-h-[85vh] overflow-y-auto hide-scrollbar" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex justify-between items-center mb-5 md:mb-6">
              <h3 className="text-xl md:text-2xl font-black text-slate-800 tracking-tight">{editingItem ? "Edit Item" : "New Item"}</h3>
            ...
            <div className="flex gap-1 md:gap-2 mb-5 md:mb-6 bg-slate-50 p-1 rounded-xl w-fit border border-slate-100">
              <button onClick={() => setModalTab("Details")} className={`px-4 md:px-5 py-2 rounded-lg text-[10px] md:text-xs font-black uppercase tracking-widest flex items-center gap-1.5 transition-all duration-300 ${modalTab === "Details" ? "bg-white text-slate-800 shadow-sm border border-slate-100" : "text-slate-400 hover:text-slate-600"}`}>
                <span className="text-xs md:text-sm">📄</span> Details
              </button>
              <button onClick={() => setModalTab("Option Groups")} className={`px-4 md:px-5 py-2 rounded-lg text-[10px] md:text-xs font-black uppercase tracking-widest flex items-center gap-1.5 transition-all duration-300 ${modalTab === "Option Groups" ? "bg-white text-[#FC687D] shadow-sm border border-rose-100" : "text-slate-400 hover:text-slate-600"}`}>
            ...
            {modalTab === "Details" ? (
              <form onSubmit={handleSave} className="space-y-4 md:space-y-5">
                <div>
                  <label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1.5 ml-1">Item Name *</label>
                ...
                <div>
                  <label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1.5 ml-1">Image URL</label>
                ...
                <div className="grid grid-cols-2 gap-3 pt-4 mt-4 border-t border-slate-100">
                  <button type="button" onClick={() => setIsModalOpen(false)} className="w-full py-3 md:py-3.5 rounded-xl bg-white border border-slate-200 text-slate-500 font-black uppercase tracking-widest text-[10px] md:text-xs hover:bg-slate-50 transition-all active:scale-95">
                    Cancel
                  </button>
                  <button type="submit" disabled={saving} className="w-full py-3 md:py-3.5 rounded-xl bg-[#FC687D] text-white font-black uppercase tracking-widest text-[10px] md:text-xs hover:bg-rose-500 transition-all shadow-sm disabled:opacity-70 active:scale-95">
              ...
              <div className="flex flex-col h-full animate-in fade-in duration-300 pb-2">
                ...
                <button type="button" className="w-full py-4 border border-dashed border-rose-200 text-[#FC687D] font-black uppercase tracking-widest text-[10px] md:text-xs rounded-xl hover:bg-rose-50 transition-all mb-6 active:scale-95">
                  + Add Option Group
                </button>
                ...
                <div className="grid grid-cols-2 gap-3 pt-4 mt-auto border-t border-slate-100">
                  ...
                  <button onClick={handleSave} disabled={saving} className="w-full py-3 md:py-3.5 rounded-xl bg-[#FC687D] text-white font-black uppercase tracking-widest text-[10px] md:text-xs hover:bg-rose-500 transition-all shadow-sm disabled:opacity-70 active:scale-95">
                    {saving ? "Saving..." : "Save Changes"}
                  </button>
                </div>
              </div>
