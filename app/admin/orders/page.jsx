...
export default function AdminLiveOrders() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <p className="text-[10px] font-black uppercase tracking-widest text-slate-700">Operations</p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div className="rounded-2xl bg-white border border-slate-200 p-4">
          <p className="text-[10px] font-black uppercase tracking-wider text-slate-500">Active Orders</p>
          <p className="text-2xl font-black text-slate-900 mt-1">{totals.count}</p>
        </div>
        <div className="rounded-2xl bg-white border border-slate-200 p-4">
          <p className="text-[10px] font-black uppercase tracking-wider text-slate-500">Active Value</p>
          <p className="text-2xl font-black text-slate-700 mt-1">{peso(totals.value)}</p>
        </div>
      </div>
      ...
      <div className="rounded-2xl bg-white border border-slate-200 shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="p-10 text-center text-sm font-semibold text-slate-500">No preparing orders.</div>
        ) : (
            ...
            {orders.map((order) => (
              <div key={order.id} className="p-4 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                <div>
                  ...
                  <p className="text-[11px] font-semibold text-slate-500 mt-1">{order.created_at ? formatDateTime(order.created_at) : ""}</p>
                </div>
                <div className="flex items-center gap-2">
                  <span className="rounded-lg bg-sky-50 border border-slate-200 px-2.5 py-1 text-[10px] font-black uppercase text-slate-700">{order.status}</span>
