...
/* ──────────────────────────────────────────────────────────────
   OneDrive / SharePoint link hiding utilities
────────────────────────────────────────────────────────────── */
const isOneDriveLink = (url = "") =>
  /(^|\/\/)(1drv\.ms|onedrive\.live\.com|sharepoint\.com|my\.sharepoint\.com)/i.test(
    String(url || "")
  );

function stripOneDriveUrls(text = "") {
  return String(text || "")
    .replace(
...
function SafeImage({ src, alt, className }) {
  if (!src || isOneDriveLink(src)) {
    return (
      <div
        className={`bg-slate-50 border border-slate-100 flex items-center justify-center text-slate-300 ${className}`}
      >
...
function SafeExternalLink({ href, className, children }) {
  if (!href || isOneDriveLink(href)) return null; // hide OneDrive/SharePoint links
  return (
        ...
        <div className="space-y-2.5 text-[11px] md:text-[13px] text-slate-600 font-normal">
          ...
          <p className="flex gap-3 items-start">
            ...
            <span>
              Store: 10AM – 12MN
              <br />
              <span className="text-slate-400 text-[10px]">Room: 10AM – 2AM</span>
            </span>
          </p>
        </div>
      </div>
    ...
    <div className="space-y-4 md:space-y-6 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-2 gap-3 md:gap-4 pb-10">
        {filtered.map((item) => {
          ...
          const imageOk = item.image_url && !isOneDriveLink(item.image_url);
          ...
          return (
            ...
            >
              <div className="h-24 md:h-32 rounded-lg md:rounded-xl overflow-hidden mb-2 border border-slate-100">
                {imageOk ? (
                  <img
                    src={item.image_url}
                    alt={stripOneDriveUrls(item.name)}
...
function LoyaltyTab({ member, setMember, user }) {
  ...
  // EDIT PROFILE fields (reduced): name, phone, city, note(birthday)
      ...
      >
        <div className="px-5 py-5 md:px-6 md:py-7 uppercase text-center border-b border-white/20 relative">
          ...
          <h3 className="text-xl md:text-2xl font-normal text-white tracking-tight">
            {stripOneDriveUrls(member?.customer_name || "Juja Member")}
          </h3>
        </div>
        ...
        <div className="px-5 py-5 md:px-6 md:py-6 space-y-3 border-b border-white/20 bg-black/5">
          {[
            ...
            // ✅ Display City instead of Address
      ...
      {showPerks && (
        ...
        >
          ...
          >
            ...
            {/* Your perks content can stay here; it contains no OneDrive links */}
            <div className="space-y-3 text-sm text-slate-700">
              <p>🌟 Earn <b>1 JUJA Point</b> for every ₱25 spent</p>
              <p>🎁 Redeem: <b>100 Points</b> = FREE reward item</p>
              <p>⏳ Rewards expire 90 days after reaching 100 points</p>
              <p>📲 Show your member barcode at checkout</p>

              {/* Example safe external link usage (will hide if OneDrive): */}
              <SafeExternalLink
                href=""
                className="text-[#FC687D] underline text-[12px]"
              >
                Learn more
              </SafeExternalLink>
            </div>
