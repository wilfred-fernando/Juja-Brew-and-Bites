...
const navItems = [
  ...
  {
    name: "POS Admin",
    path: "/admin/pos", // ✅ make this consistent
    icon: "🖥️",
    submenu: [
...
function isActiveItem(item, pathname) {
  ...
  if (item.submenu?.some((s) => pathname === s.path || pathname.startsWith(s.path + "/"))) return true;
  return false;
}
...
export default function Sidebar() {
  ...
  // ✅ hide sidebar on login routes
  if (pathname === "/login" || pathname.startsWith("/admin/login")) return null;

  useEffect(() => {
    const active = navItems.find((item) => isActiveItem(item, pathname));
    if (active?.submenu) setOpenSection(active.name);
  }, [pathname]);
            ...
            <div key={item.name} className="flex flex-col w-full">
              {hasSub ? (
                <button
                  ...
                  className={`flex items-center justify-between px-4 py-3.5 rounded-2xl transition-all outline-none relative ${
              ...
              {hasSub && (
                <div className={`overflow-hidden transition-all duration-300 ${isOpen ? "max-h-96 opacity-100 mt-1 mb-4" : "max-h-0 opacity-0"}`}>
                  <div className="ml-12 border-l border-slate-100 flex flex-col gap-0.5">
                        ...
                        >
                          <div
                            className={`absolute left-0 top-1/2 -translate-y-1/2 h-[1px] transition-all ${
                              isSubActive ? "bg-[#FC687D] w-4" : "bg-gray-200 w-2 group-hover:bg-gray-400"
                            }`}
                          />
                          {sub.name}
                        </Link>
